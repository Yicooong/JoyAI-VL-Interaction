...
| `SAFEVL_CACHE_DIR` | 未设置 | 将推理结果、帧/上传缓存和 HTML 产物统一迁移到该根目录；未设置时完全保留原有位置。 |
| `SAFEVL_LOG_DIR` | 未设置 | 将文件日志和 PID 文件迁移到该根目录；目前用于摘要 vLLM 服务。 |
...
例如，`SAFEVL_CACHE_DIR=/data/safevl/cache SAFEVL_LOG_DIR=/data/safevl/logs bash services/scripts/run.sh all` 会将 WebInfer 数据保存到 `cache/webinfer`、WebUI 数据保存到 `cache/webui`，摘要日志保存到 `logs/webinfer`。原有细粒度变量（`LIVE_ADAPTER_SAVE_ROOT`、`FRAME_SAVE_DIR`、`LIVE_VLM_UPLOAD_DIR`、`LIVE_VLM_HTML_ARTIFACT_DIR`、`LOG_DIR`）优先级更高。
